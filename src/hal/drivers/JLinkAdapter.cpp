...
namespace JTAG {
    ...
    // Función helper para buscar la DLL de J-Link con caché
    // searchRecursive() eliminado - era demasiado lento
    std::string JLinkAdapter::findJLinkDLL() {
        ...
        // Deep recursive search eliminated (was too slow)
        // If DLL not found with fast strategies above, give up
    ...
    bool JLinkAdapter::open() {
        ...
        // Clock speed se configura vía setClockSpeed() después del open()
        // No hardcodear velocidad aquí
        ...
        bool exitShift)
    {
        ...
        // Forzar flush del buffer USB (crítico para UPDATE-DR inmediato)
        if (pJLINK_JTAG_SyncBits) {
            pJLINK_JTAG_SyncBits();
        } else {
            // Fallback: lectura de 1 bit con buffers separados para forzar flush
            uint8_t dummyTDI = 0, dummyTDO = 0, dummyTMS = 0;
            pJLINK_JTAG_StoreGetRaw(&dummyTDI, &dummyTDO, &dummyTMS, 1);
        }
