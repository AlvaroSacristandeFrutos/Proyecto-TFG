...
namespace JTAG {
    ...
    bool ScanController::initialize() {
        ...
        // IEEE 1149.1 Sección 7.2.1: Aplicar estado inicial con Update-DR
        // Esto "congela" el circuito con los valores precargados inmediatamente
        if (!engine->applyChanges()) {
            std::cerr << "[ScanController] Failed to apply initial EXTEST state\n";
            return false;
        }

        std::cout << "[ScanController] EXTEST loaded and initial state applied\n";

        // ========== FIN SECUENCIA IEEE 1149.1 ==========

        // Crear worker y moverlo al thread
        scanWorker = new ScanWorker(engine.get(), deviceModel.get());
        scanWorker->setPollInterval(pollIntervalMs);  // Inicializar intervalo configurable
    ...
    std::optional<PinLevel> ScanController::getPin(const std::string& pinName) const {
        ...
        // MODE-AWARE: Comportamiento según modo de operación
        auto currentMode = engine->getOperationMode();

        if (currentMode == BoundaryScanEngine::OperationMode::EXTEST ||
            currentMode == BoundaryScanEngine::OperationMode::INTEST) {
            // En EXTEST/INTEST: Leer del buffer de ESCRITURA (bsr - lo que enviamos)
            // Esto muestra el estado que estamos aplicando a los pines físicos
            if (pinInfo->outputCell != -1) {
                return engine->getPin(pinInfo->outputCell);
            }
            // Fallback: si solo tiene inputCell (raro en EXTEST), leerlo
            if (pinInfo->inputCell != -1) {
                return engine->getPinReadback(pinInfo->inputCell);
            }
        } else {
            // En SAMPLE/BYPASS: Leer del buffer CAPTURADO (bsrCapture - lo que el chip envió)
            // Esto muestra el estado real del chip
            if (pinInfo->inputCell != -1) {
                return engine->getPinReadback(pinInfo->inputCell);
            }
            // Fallback: si solo tiene outputCell, leerlo
            if (pinInfo->outputCell != -1) {
                return engine->getPin(pinInfo->outputCell);
            }
        }
    ...
    void ScanController::setSamplesPerSecond(int samplesPerSec) {
        if (scanWorker) {
            scanWorker->setSamplesPerSecond(samplesPerSec);
        }
    }
